...
class Booking(models.Model):
    ...
    booking_ref = models.CharField(primary_key=True, max_length=32, null=False, editable=False, default='')
...
class BookingItem(models.Model):
    bookings = models.ForeignKey(Booking, null=False, blank=False, on_delete=models.CASCADE, related_name='bookingitem')
    ...
    bookingitem_total = models.DecimalField(max_digits=10, decimal_places=2, null=False, blank=False, editable=False)

    def save(self, *args, **kwargs):
        """
       Overrides the original save method to set the bookingitem total and updates the booking total
        """
        self.bookingitem_total = Decimal(self.experience.price * self.number_of_people)
        super().save(*args, **kwargs)
